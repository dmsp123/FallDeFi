...
% feature_mat = zeros(length(stft_cell_array)*2,6);
...
% N=4;% 800 -4.2  600 -4.2 400 -4.2 200
...
for i = 1 : fall_cell_array_size
    i
    label_num(i)
    stft_mat = stft_cell_array{i};    stft_mat1 = stft_cell_array{i};
    noise_lvl=2*max(max(stft_mat(250:500,:)));
    stft_mat(stft_mat<=noise_lvl)=0;
    ...
    %% Binary image
    stft_mat_bin = stft_mat;
    stft_mat_bin(stft_mat_bin > noise_lvl) = 1;
    
    
    BW2 = +bwareaopen(stft_mat_bin,200);%logical to double conversion
    
    h=figure;        set(h, 'Visible', 'on');
    surf(BW2,'edgecolor','none')
    colormap(gray); axis([0 10000 0 110])
    %     h=figure;     set(h, 'Visible', 'on');
    %
    ...
    %         [~, r] = max( L==0, [], 1 );
    [rowIdx,colIdx] = find(stft_mat_nooutl);
    ...
    event_thr =  2*max(max(stft_mat1(250:500,:)))*(PBC_freq_max-PBC_freq_min);
    ...
    if (event_thr < max(PBC))
        ...
        rem_ind=val_down_ind-val_up_ind-100<1;
        val_down_ind(rem_ind)= [];
        val_up_ind(rem_ind)= [];
        for iv = 1 : length(val_down_ind)% or val_up_ind
            ...
            if event_dur>100
                m = m + 1;
                feature_mat = zeros(6,floor((event_int(2)-event_int(1))/100));
                ...
                %
                level_mean_energy = zeros(12,(event_int(2)-event_int(1))+101);
                
                for v = 1 : 12
                    if(event_int(1)>100)
                        level_mean_energy(v,:) = mean(stft_mat((v-1)*25+1:25*v,event_int(1)-100:event_int(2)),1);
                    else
                        level_mean_energy(v,:) = mean(stft_mat((v-1)*25+1:25*v,[event_int(1):100+event_int(1) event_int(1)+1:event_int(2)]),1);
                    end
                end
                
                for v = 1 : floor((event_int(2)-event_int(1))/100)
                    feature_mat(:,v)= [ mean(index_ext((v-1)*100+1:v*100));...
                        mean(index_torso((v-1)*100+1:v*100));...
                        mean(index_leg((v-1)*100+1:v*100));...
                    ...
                    if(label_num(i)==6)
                        if any(falls_list(:,1)-i)&&iv==falls_list(falls_list(:,1)-i==0,2)
            ...
            %% event lines
            h = figure(6);set(h, 'Visible', 'on'); plot3(repmat(event_int(1),[1,109]),1:109,repmat(z_max,[1,109]),'color','k', 'linewidth',2, 'linestyle','-');
            h = figure(6); set(h, 'Visible', 'on');
            plot3(repmat(event_int(2),[1,109]),1:109,repmat(z_max,[1,109]),'color','k', 'linewidth',2, 'linestyle','-');
