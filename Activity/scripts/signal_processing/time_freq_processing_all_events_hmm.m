...
% fname = 'C:\Users\R00119847\Documents\MATLAB\csi_tool_matlab\Activity\Figures\Corridor\corridor1';
% fname = 'C:\Users\R00119847\Documents\MATLAB\csi_tool_matlab\Activity\Figures\Bedrooms';
% fname = 'C:\Users\R00119847\Documents\MATLAB\csi_tool_matlab\Activity\Figures\Lab2';
...
feature_cell = cell(length(stft_cell_array)*2,1);
...
N=3;% 800 -4.2  600 -4.2 400 -4.2 200
...
bkg_noise_thrind        = zeros(1,length(stft_cell_array));
for i = 1 : length(stft_cell_array)
    stft_mat            = stft_cell_array{i};
    pd                  = fitdist(reshape(real(log10(stft_mat(300:end,:))),[],1),'normal');
...
for i = 1 : length(stft_cell_array)
    ...
    stft_mat = stft_cell_array{i};
        ...
        set(h, 'Visible', 'on');
        s=surf(imresize(stft_mat,[500 1000]),'edgecolor','none');
        colormap(jet)
        axis([0 1000 0 110])
    ...
    % kitchen 47:lose balance, lose conscious 57, slip:69, trip:79
        ...
        colormap(jet)
        axis([0 1000 0 110]); caxis([100,5000])
        set(s,'EdgeColor','interp');
    ...
    if(~isempty(ind))
        ...
        if (event_thr < max(PBC))
            ...
            for iv = 1 : length(val_down_ind)% or val_up_ind
                ...
                event_dur    = val_down_ind(iv) - val_up_ind(iv);
                
                if event_dur>200
                    m = m + 1;
                    feature_mat = zeros(30,floor((event_int(2)-event_int(1))/200));
                    ...
                    %                     end
                    %                 end
                    index_torso = zeros(1,event_dur);
                    index_leg   = zeros(1,event_dur);
                    index_ext   = zeros(1,event_dur);%%extreme frequency
                    for ii=1:event_dur+1
                        %    [c index(ii)] = min( abs(log10(cumsum(stft_mat_uncl(min(r):r(ii),ii))) - log10(sum(stft_mat_uncl(min(r):r(ii),ii))/2)));
                        %       [c index(ii)] = min( abs((cumsum(stft_mat_uncl(1:r(ii),ii))) - sum(stft_mat_uncl(1:r(ii),ii))/2));
                        [~, index_ext(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii)+1,ii))) - sum((stft_mat_nooutl(1:r(ii),ii)))));

                        if r(ii)~=500
                            [~, index_torso(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii)+1,ii))) - sum((stft_mat_nooutl(1:r(ii)+1,ii)))/2));
                            [~, index_leg(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii)+1,ii))) - sum((stft_mat_nooutl(1:r(ii)+1,ii)))*.95));
%                             [~, index_ext(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii)+1,ii))) - sum((stft_mat_nooutl(1:r(ii)+1,ii)))*.05));

                            %               [c index(ii)] = min( abs((cumsum(stft_mat_uncl(1:r(ii),ii))) - sum(stft_mat_uncl(1:r(ii),ii))/2));
                        else
                            [~, index_torso(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii),ii))) - sum((stft_mat_nooutl(1:r(ii),ii)))/2));
                            [c, index_leg(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii),ii))) - sum((stft_mat_nooutl(1:r(ii),ii)))*.95));
%                           [~, index_ext(ii)] = min(abs( cumsum((stft_mat_nooutl(1:r(ii)+1,ii))) - sum((stft_mat_nooutl(1:r(ii),ii)))*.05));
...
%                     
                    level_mean_energy = zeros(12,(event_int(2)-event_int(1))+201);
                    
                    for v = 1 : 12
                        if(event_int(1)>200)
                        level_mean_energy(v,:) = mean(stft_mat((v-1)*25+1:25*v,event_int(1)-200:event_int(2)),1);
                        else
                        level_mean_energy(v,:) = mean(stft_mat((v-1)*25+1:25*v,[event_int(1):200+event_int(1) event_int(1)+1:event_int(2)]),1);
                        end
                    end
                    
                    for v = 1 : floor((event_int(2)-event_int(1))/200)
                        feature_mat(:,v)= [ mean(index_ext((v-1)*200+1:v*200));...
                            mean(index_torso((v-1)*200+1:v*200));...
                            mean(index_leg((v-1)*200+1:v*200));...
                            mean(level_mean_energy(:,(v)*200+1:(v+1)*200),2);...
                            mean(level_mean_energy(:,(v)*200+1:(v+1)*200),2)-mean(level_mean_energy(:,(v-1)*200+1:v*200),2);...
                            label_num(i);i;2];
                        %                     feature_mat = ...
                        ...
                        %                         mean(index_ext);...
                        %                         spectral_entropy1030,spectral_entropy110;...
                        %                         dim_val;...
                        ...
                        if(label_num(i)==6)
                            if iv==I
                                feature_mat(end-2,v) = 6;
                                feature_mat(end,v)   = 6;
                            else
                                feature_mat(end-2,v) = 2;
                            end
                        end
                        feature_cell{m} = feature_mat;
                    end
        ...
        m=m+1;
        feature_mat([end-2,end-1,end]) = [2,i, label_num(i)];
        feature_cell{m} = feature_mat;
    end
    close all
end
...
feature_cell        = feature_cell(~cellfun('isempty',feature_cell))  ;
j=0;k=0;
for i = 1:length(feature_cell)
    feature_mat=feature_cell{i};
     if(feature_mat(end,1)==6)
         j=j+1;
     else
         k=k+1;
     end
 end
feature_fall_cell    = cell(j,1);
feature_fall_mat     = zeros(27,49,j);
feature_nonfall_mat     = zeros(27,49,j);

feature_nonfall_cell = cell(k,1);
j=0;k=0;
 for i = 1:length(feature_cell)
    feature_mat=feature_cell{i};
     if(feature_mat(end,1)==6)
         j=j+1;
         feature_fall_cell{j}=feature_mat(1:end-3,:);
         feature_fall_mat(:,1:size(feature_mat,2),j)= feature_mat(1:end-3,:);
         feature_fall_mat(:,size(feature_mat,2)+1:end,j)= repmat(feature_mat(1:end-3,end),1,49-size(feature_mat,2));
     else
         k=k+1;
                  feature_nonfall_mat(:,1:size(feature_mat,2),k)= feature_mat(1:end-3,:);
 feature_nonfall_mat(:,size(feature_mat,2)+1:end,k)= repmat(feature_mat(1:end-3,end),1,49-size(feature_mat,2));
                  feature_nonfall_cell{k}=feature_mat(1:end-3,:);
     end
 end
%  feature_fall_cell        = feature_fall_cell(~cellfun('isempty',feature_fall_cell))  ;
% feature_nonfall_cell        = feature_nonfall_cell(~cellfun('isempty',feature_nonfall_cell))  ;
